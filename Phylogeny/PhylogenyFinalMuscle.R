...
dna_sarsCov2_start_30000 <- readDNAStringSet(
  file="~/Desktop/Gepoliano/UFOB/sequencesfilogeniaData/bahia_combined_30000_muscle.fasta"
  )
...
sars <- read.dna(
  "~/Desktop/Gepoliano/UFOB/sequencesfilogeniaData/bahia_combined_30000_muscle.fasta", 
  format="fasta")
...
# When using the Muscle alignment file, the object to be converted actually has 30561 bp in total (with the gaps)
dm  <- dist.ml(as.DNAbin(dna_sarsCov2_start_30000))
...
pdf(file = "PhylogenyFigures/Teixeira_Tree_UPGMA_muscle.pdf",
    width = 20, 
    height = 7)
plot(treeUPGMA, main="UPGMA", col="red")
dev.off()

pdf(file = "PhylogenyFigures/Teixeira_Tree_NJ_Unrooted_muscle.pdf",
    width = 15, 
    height = 7)
plot(treeNJ, "unrooted", main="NJ Unrooted")
dev.off()

pdf(file = "PhylogenyFigures/Teixeira_Tree_NJ_muscle.pdf",
    width = 15, 
    height = 7)
plot(treeNJ, main="NJ")
dev.off()
